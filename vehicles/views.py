...
def maintenance_list(request):
    user = request.user
    if user.role == user.Role.CUSTOMER:
        records = MaintenanceRecord.objects.filter(vehicle__owner=user)
    elif user.role == user.Role.TECHNICIAN:
        records = MaintenanceRecord.objects.filter(technician__user=user)
    else:
...
def appointment_list(request):
    user = request.user
    if user.role == user.Role.CUSTOMER:
        appointments = Appointment.objects.filter(vehicle__owner=user)
    elif user.role == user.Role.TECHNICIAN:
        appointments = Appointment.objects.filter(technician__user=user)
    else:
